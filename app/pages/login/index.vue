...
<template>
  <div
    class="min-h-screen bg-slate-50 font-sans text-slate-900 flex flex-col justify-center items-center relative overflow-hidden selection:bg-emerald-200 selection:text-emerald-900"
  >
    <div
      class="absolute inset-0 z-0 bg-linear-to-br from-slate-50 via-emerald-50/30 to-teal-100/40 pointer-events-none"
    ></div>
    <div class="absolute inset-0 z-0 overflow-hidden pointer-events-none">
      <div
        v-for="n in 5"
        :key="n"
        class="absolute bg-emerald-400/20 rounded-full blur-2xl animate-float"
        :style="{
    ...
    <div class="relative z-10 w-full max-w-md px-6 animate-fade-in-up">
      <NuxtLink
        to="/"
        class="flex flex-col items-center gap-2 mb-8 group cursor-pointer hover:scale-105 transition-transform"
      >
        <div
          class="w-12 h-12 rounded-2xl bg-emerald-600 flex items-center justify-center shadow-lg shadow-emerald-600/30 group-hover:bg-emerald-500 transition-colors"
        >
          <Icon name="mdi:leaf" class="text-white text-3xl" />
        </div>
        <span
          class="text-2xl font-display font-extrabold tracking-tight text-slate-900"
          >NyawitAI</span
        >
      </NuxtLink>

      <div
        class="p-8 rounded-3xl bg-white/60 backdrop-blur-xl border border-white/80 shadow-2xl shadow-emerald-900/10 w-full"
      >
        <div class="text-center mb-8">
          ...
          <p class="text-sm text-slate-500">
            {{
              isLogin
                ? "Masuk untuk melihat analisis kebun sawit Anda."
                : "Daftar sekarang untuk revolusi investasi sawit."
            }}
          </p>
        </div>
        ...
        <form @submit.prevent="handleSubmit" class="space-y-5">
          ...
          <div
            v-if="errorMessage"
            class="p-3 rounded-xl bg-red-50 border border-red-200 text-red-700 text-sm font-medium flex items-center gap-2"
          >
          ...
          <div
            v-if="successMessage"
            class="p-3 rounded-xl bg-emerald-50 border border-emerald-200 text-emerald-700 text-sm font-medium flex items-center gap-2"
          >
          ...
          >
            <label class="text-sm font-semibold text-slate-700 block ml-1"
              >Nama Lengkap</label
            >
          ...
          >
            ...
            <Icon
              ...
              :name="isLogin ? 'mdi:login' : 'mdi:account-plus-outline'"
        ...
        <a
          ...
          class="w-full mt-6 py-3 px-6 rounded-xl bg-white border hover:bg-slate-50 text-slate-700 font-semibold transition-all flex justify-center items-center gap-3 cursor-pointer"
        >
