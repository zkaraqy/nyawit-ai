...
const joinWaitlist = () => {
  ...
  setTimeout(() => {
    ...
    alert('Fitur waitlist akan segera hadir. Terima kasih!')
    email.value = ''
  }, 1500)
}
</script>
...
<template>
  <div class="min-h-screen bg-slate-50 font-sans text-slate-900 overflow-x-hidden selection:bg-emerald-200 selection:text-emerald-900">
    <!-- Loading Screen -->
    <div v-if="appLoading" class="fixed inset-0 z-100 bg-white flex flex-col items-center justify-center transition-opacity duration-500">
      <div class="relative w-24 h-24 mb-4">
         <Icon name="mdi:leaf" class="text-emerald-600 text-6xl absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 animate-pulse" />
         <div class="absolute inset-0 border-4 border-emerald-100 rounded-full"></div>
         <div class="absolute inset-0 border-4 border-emerald-500 rounded-full border-t-transparent animate-spin"></div>
      </div>
      <h2 class="text-xl font-bold font-display text-slate-800 tracking-wider">NyawitAI</h2>
      <p class="text-slate-500 text-sm mt-2 animate-pulse">Memuat Peta Interaktif...</p>
    </div>

    <!-- Navbar -->
    <nav class="fixed top-0 left-0 right-0 z-50 px-6 py-4 flex justify-between items-center backdrop-blur-md bg-emerald-900/95 border-b border-white/10 shadow-lg shadow-emerald-900/20 overflow-hidden">
          ...
          <svg width="100%" height="100%" xmlns="http://www.w3.org/2000/svg">
              <defs>
                  <pattern id="palm-pattern" x="0" y="0" width="40" height="40" patternUnits="userSpaceOnUse">
                       <path d="M20,20 Q25,5 35,5 Q25,10 20,20 Q15,30 5,30 Q15,25 20,20" fill="none" stroke="currentColor" stroke-width="1.5" class="text-white"/>
                       <path d="M20,20 Q15,35 5,35 Q15,30 20,20 Q25,10 35,10 Q25,15 20,20" fill="none" stroke="currentColor" stroke-width="1.5" class="text-white" transform="rotate(180 20 20)"/>
                  </pattern>
              </defs>
      ...
      <button class="relative z-10 px-5 py-2 text-sm font-bold text-emerald-900 bg-white rounded-full hover:bg-emerald-50 transition-all shadow-lg hover:shadow-xl hover:scale-105 active:scale-95">
        Masuk / Daftar
      </button>
    </nav>
    ...
    <section class="relative w-full min-h-screen lg:h-screen overflow-hidden flex items-center pt-24 lg:pt-10">
      ...
      <!-- Overlay Gradient -->
      <div class="absolute inset-0 z-10 bg-linear-to-r from-slate-50/90 via-slate-50/60 to-transparent pointer-events-none"></div>

      <!-- Floating Dots Animation -->
      <div class="absolute inset-0 z-10 overflow-hidden pointer-events-none">
          <div v-for="n in 5" :key="n" 
              class="absolute bg-emerald-400/20 rounded-full blur-xl animate-float"
              :style="{
      ...
      <!-- Content -->
      <div class="relative z-20 w-full max-w-7xl mx-auto px-6 flex flex-col lg:grid lg:grid-cols-2 gap-12 items-center h-full">
        
        <!-- Left Column: Text & Form -->
        <div class="space-y-2 animate-fade-in-up order-2 lg:order-1 w-full pb-10 lg:pb-0">
          <div class="space-y-4">
            <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-100/80 border border-emerald-200 text-emerald-700 text-xs font-bold uppercase tracking-wider backdrop-blur-sm">
            ...
            <h1 class="text-4xl lg:text-5xl font-display font-extrabold text-slate-900 leading-tight tracking-tight">
              Investasi Sawit <br/>
              <span class="text-transparent bg-clip-text bg-linear-to-r from-emerald-600 to-teal-500">Cerdas & Terukur</span>
            </h1>
            <p class="text-lg text-slate-600 max-w-lg leading-relaxed">
              Dapatkan analisis kelayakan lahan berbasis data agroklimatologi real-time dan prediksi hasil panen presisi dengan teknologi AI kami.
            </p>
          </div>

          <!-- Glassmorphism Form Card -->
          <div class="p-6 rounded-2xl bg-white/40 backdrop-blur-xl border border-white/50 shadow-2xl shadow-emerald-900/5 max-w-md w-full">
            <form @submit.prevent="handleAnalyze" class="space-y-5">
              ...
              >
                ...
                <Icon v-if="!analysisLoading" name="mdi:arrow-right" class="group-hover:translate-x-1 transition-transform" />
        ...
        <div class="w-full h-[400px] relative pointer-events-auto lg:-mr-20 bg-white/30 backdrop-blur-md rounded-3xl shadow-2xl shadow-emerald-900/20 border border-white/50 overflow-hidden order-1 lg:order-2 top-0">
             <ClientOnly>
               <HeroMap3D />
             </ClientOnly>
        </div>
    ...
    <section class="py-24 px-6 bg-slate-50 relative">
        <div class="max-w-7xl mx-auto">
             <div class="text-center mb-16 space-y-4">
                <h2 class="text-3xl lg:text-4xl font-display font-bold text-slate-900">Mengapa <span class="text-emerald-600">NyawitAI?</span></h2>
                <p class="text-slate-500 max-w-2xl mx-auto text-lg">Platform pintar yang menggabungkan keahlian agronomi dengan kecerdasan buatan.</p>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                ...
                <div class="bg-white p-8 rounded-2xl shadow-sm border border-slate-100 hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
                    <div class="w-14 h-14 bg-emerald-100 rounded-xl flex items-center justify-center text-emerald-600 text-3xl mb-6">
                        <Icon name="mdi:chart-line-variant" />
                    </div>
                    <h3 class="text-xl font-bold font-display text-slate-900 mb-3">Keputusan Berbasis Data</h3>
                    <p class="text-slate-600 leading-relaxed">Hindari spekulasi. Setiap rekomendasi didukung oleh analisis big data historis dan prediktif.</p>
                </div>
                <!-- Benefit 2 -->
                <div class="bg-white p-8 rounded-2xl shadow-sm border border-slate-100 hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
                    ...
                    <p class="text-slate-600 leading-relaxed">Model kami memperhitungkan perubahan iklim jangka panjang untuk memastikan keberlanjutan investasi Anda.</p>
                </div>
                <!-- Benefit 3 -->
                <div class="bg-white p-8 rounded-2xl shadow-sm border border-slate-100 hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
                    <div class="w-14 h-14 bg-emerald-100 rounded-xl flex items-center justify-center text-emerald-600 text-3xl mb-6">
                    ...
                    <p class="text-slate-600 leading-relaxed">Dapatkan estimasi hasil panen dan keuntungan finansial yang realistis sebelum Anda menanam satu benih pun.</p>
                </div>
            </div>
        </div>
    </section>
    ...
    <section class="py-24 px-6 bg-white relative overflow-hidden">
        <!-- Decorative blobs -->
        <div class="absolute top-0 right-0 w-[500px] h-[500px] bg-emerald-50/50 rounded-full blur-3xl -translate-y-1/2 translate-x-1/2"></div>
        <div class="absolute bottom-0 left-0 w-[500px] h-[500px] bg-teal-50/50 rounded-full blur-3xl translate-y-1/2 -translate-x-1/2"></div>

        <div class="relative max-w-7xl mx-auto">
            <div class="text-center mb-16 space-y-4">
                <h2 class="text-3xl lg:text-4xl font-display font-bold text-slate-900">3 Langkah <span class="text-emerald-600">Investasi Cerdas</span></h2>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-12 relative z-10">
                <!-- Step 1 -->
                <div class="bg-slate-50/80 backdrop-blur-sm rounded-2xl p-8 border border-slate-100 hover:border-emerald-200 transition-colors group text-center space-y-6">
                    <div class="w-20 h-20 mx-auto rounded-2xl bg-white shadow-sm flex items-center justify-center text-4xl text-emerald-600 border border-slate-100 group-hover:scale-110 transition-transform duration-300">
                        <Icon name="mdi:file-document-edit-outline" />
                    </div>
                    <div class="space-y-2">
                        <h3 class="text-xl font-bold font-display text-slate-900">1. Input Parameter</h3>
                        <p class="text-slate-600 leading-relaxed">Cukup masukkan luas area dan lokasi target. Sistem kami akan langsung mengidentifikasi koordinat geosparsial.</p>
                    </div>
                </div>

                <!-- Step 2 -->
                <div class="bg-slate-50/80 backdrop-blur-sm rounded-2xl p-8 border border-slate-100 hover:border-emerald-200 transition-colors group text-center space-y-6">
                    <div class="w-20 h-20 mx-auto rounded-2xl bg-white shadow-sm flex items-center justify-center text-4xl text-teal-600 border border-slate-100 group-hover:scale-110 transition-transform duration-300">
                        <Icon name="mdi:brain" />
                    </div>
                    <div class="space-y-2">
                        <h3 class="text-xl font-bold font-display text-slate-900">2. Analisis Deep Learning</h3>
                        <p class="text-slate-600 leading-relaxed">Engine AI memproses ribuan data points termasuk jenis tanah, curah hujan, dan elevasi secara instan.</p>
                    </div>
                </div>
                ...
                <div class="bg-slate-50/80 backdrop-blur-sm rounded-2xl p-8 border border-slate-100 hover:border-emerald-200 transition-colors group text-center space-y-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <p class="text-slate-600 leading-relaxed">Terima peta kelayakan 3D interaktif dan laporan proyeksi finansial komprehensif untuk panduan Anda.</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    <section class="py-24 px-6 bg-slate-900 relative overflow-hidden">
        <!-- Glow effects -->
        <div class="absolute top-1/2 left-1/4 w-[400px] h-[400px] bg-emerald-500/20 rounded-full blur-[100px] -translate-y-1/2"></div>
        <div class="absolute top-1/2 right-1/4 w-[400px] h-[400px] bg-teal-500/20 rounded-full blur-[100px] -translate-y-1/2"></div>
        
        <div class="relative z-10 max-w-3xl mx-auto text-center space-y-8">
            <h2 class="text-4xl lg:text-5xl font-display font-bold text-white tracking-tight">Jadilah yang Pertama <br/>Mencoba <span class="text-emerald-400">Revolusi Agrotech</span></h2>
            <p class="text-slate-400 text-lg max-w-xl mx-auto">Kami sedang mempersiapkan peluncuran publik. Masukkan email Anda untuk mendapatkan notifikasi dan akses awal eksklusif.</p>
    ...
    <footer class="bg-slate-950 pt-20 pb-10 px-6 text-slate-400 relative overflow-hidden">
        ...
        <div class="absolute bottom-0 left-1/2 -translate-x-1/2 opacity-5 pointer-events-none w-full max-w-4xl flex justify-center items-end overflow-hidden">
           <svg viewBox="0 0 500 200" class="w-full h-auto text-emerald-500 fill-current">
               <path d="M250,200 L250,150 Q230,120 200,100 Q150,80 100,120 Q120,60 180,50 Q240,40 250,100 Q260,40 320,50 Q380,60 400,120 Q350,80 300,100 Q270,120 250,150 Z" />
