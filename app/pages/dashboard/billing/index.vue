...
const packages = [
  { id: 1, name: 'Paket Starter', tokens: 5, price: 'Rp 2.500.000', popular: false, desc: 'Cocok untuk investor mandiri.' },
  { id: 2, name: 'Paket Korporasi', tokens: 20, price: 'Rp 8.000.000', popular: true, desc: 'Pilihan tepat untuk perusahaan perkebunan.' },
  { id: 3, name: 'Paket Enterprise', tokens: 50, price: 'Rp 17.500.000', popular: false, desc: 'Untuk ekspansi lahan skala masif.' },
]

const transactions = [
  { id: 'INV-20260401', date: '01 Apr 2026', desc: 'Top-up Paket Korporasi', amount: '+20 Token', price: 'Rp 8.000.000', status: 'Lunas' },
  { id: 'TRX-20260405', date: '05 Apr 2026', desc: 'Analisis Lahan Riau (350 Ha)', amount: '-1 Token', price: '-', status: 'Berhasil' },
  { id: 'TRX-20260410', date: '10 Apr 2026', desc: 'Analisis Lahan Sumut (1200 Ha)', amount: '-1 Token', price: '-', status: 'Berhasil' },
]
</script>
  ...
  <div class="space-y-8 max-w-6xl mx-auto">
    ...
    <div class="space-y-4">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div v-for="pkg in packages" :key="pkg.id" 
          class="relative bg-white border-2 rounded-2xl p-6 transition-all hover:shadow-xl hover:-translate-y-1 cursor-pointer flex flex-col"
    ...
    <div class="space-y-4">
      <div class="flex items-center justify-between">
        ...
        <button class="text-emerald-600 hover:text-emerald-700 font-bold text-sm flex items-center gap-1">
          Unduh Laporan <Icon name="mdi:download" />
        </button>
      </div>
        ...
        <table class="w-full text-left">
          ...
          <tbody class="divide-y divide-slate-100 text-sm">
            <tr v-for="trx in transactions" :key="trx.id" class="hover:bg-slate-50 transition-colors">
              <td class="p-5 text-slate-500 font-medium">{{ trx.date }}</td>
              <td class="p-5 font-bold text-slate-800">{{ trx.desc }}</td>
              <td class="p-5 font-bold" :class="trx.amount.includes('+') ? 'text-emerald-600' : 'text-red-500'">
                {{ trx.amount }}
              </td>
              <td class="p-5 text-slate-600 font-medium">{{ trx.price }}</td>
              <td class="p-5">
                <button v-if="trx.id.includes('INV')" class="flex items-center gap-1 text-slate-500 hover:text-emerald-600 font-medium transition-colors">
                  <Icon name="mdi:file-pdf-box" class="text-lg" /> Unduh
                </button>
